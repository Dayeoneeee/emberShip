...
public class MemberShipService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String email) throws UsernameNotFoundException {
        ...
        //비교해서 맞다면 로그인한다.
        log.info("유저디테일 서비스로 들어온 이메일 :" + email);   // <-안들어왔다면 input 창으로 넣은 값이 도달을 못했다는 뜻
        MemberShip memberShip =
        this.membershipRepository.findByEmail(email);
        //이메일로 검색해서 가져온 값이 없다면, 회원가입이 되어있지 않다면 예외 처리를 해주겠다.
        //try catch 문으로 다른 화면으로 덜리던 메시지를 가지고 로그인창으로 보내던 컨트롤러 창에서 알아서 해주겠지.
        //미래의 내가
        if (memberShip == null){
            throw new UsernameNotFoundException("사용자를 찾을 수 없습니다.");
        }
        //예외처리가 되지 않았다면
        log.info("현재 찾은 회원 정보" + memberShip);
